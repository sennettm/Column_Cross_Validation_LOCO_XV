...
def lco(direct, file, form):
    ...
    for i in range(len(alignment1[0].seq)):

        newfile=re.sub(r'([.].*)','_sc_'+str(i+1)+'.'+str(form1), str(file1))
        tmp_aln=[]

        #getting seq name and new sequence
        for j in range(len(alignment1)):
            tmp_aln.append('>'+str(alignment1[j].id)+'\n')
            tmp_seq=str(alignment1[j].seq)
            k=i+1
            tmp_seq=tmp_seq[:i]+tmp_seq[k:]
            tmp_seq_lst=list(tmp_seq)
            for m in range(0, len(tmp_seq_lst), 70):
                n=m+70
                tmp_aln.append("".join(tmp_seq_lst[m:n])+"\n")

        with open(os.path.join(direct, newfile), 'w+') as new_aln:
            for line in tmp_aln:
                new_aln.write(str(line))
            new_aln.close()
...
parser.add_argument("-X", "--Cross_Val", action="store_true",
        help="del a single column at a time in an MSA, output new alignment for each deletion")
...
if args.explode:
    ...
    lco(args.direct, args.MSA, args.form1)
